...
if not csv_path.exists():
    csv_path = Path("model/S1_training.csv")
...
adapter = TCN_EKF_ModelAdapter(checkpoint_path="model/best_model_real.pt")
...
for name, (metrics, status) in results.items():
    ...
    for sm in metrics.segment_metrics:
        print(f"    Segment {sm.interval.name} ({sm.blackout_duration_s:.1f}s, dist={sm.distance_traveled_m:.2f}m): Error={sm.end_drift_distance_m:.2f}m, Drift={sm.drift_percentage:.2f}% (Pass: {sm.passed_target})")
